...
exports.getShifts = async (employeeId) => {
    try {
        ...
        let shifts = await Shift.find({ employeeId }).sort({ startTime: -1 });
        for (let i = 0; i < shifts.length; i++) {
            shifts[i] = shifts[i]._doc;
            if (shifts[i].startTime) {
                let startTime = moment(shifts[i].startTime);
                let startTimeOffset = startTime.utcOffset();
                startTimeOffset = startTimeOffset / 60;
                startTime = startTime.toDate();
                startTime.setHours(startTime.getHours() + startTimeOffset);
                shifts[i].startTime = new Date(startTime);
            }
            if (shifts[i].endTime) {
                let endTime = moment(shifts[i].endTime);
                let endTimeOffset = endTime.utcOffset();
                endTimeOffset = endTimeOffset / 60;
                endTime = endTime.toDate();
                endTime.setHours(endTime.getHours() + endTimeOffset);
                shifts[i].endTime = new Date(endTime);
            }
        }
        if (!shifts) {
